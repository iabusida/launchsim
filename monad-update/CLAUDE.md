...
Engine math is chain-agnostic: the quote asset is generic (MON on Monad, SOL on Solana). Code and docs say "quote", not "SOL".
...
Dependency direction is one-way: `cli → share/registry/report/adapters → core`. `contracts/` is independent; `registry` consumes its ABI from Foundry's build output. `core` depends on nothing internal. `testkit` is a devDependency only.
...
**Monad hackathon build** (`docs/09-roadmap-mvp.md`, `docs/13-hackathon-submission.md`). M0–M4 (engine, markets, actors, mechanics, checks, report) are done or in progress on the chain-agnostic plan. Next: **M5-Monad** — Nad.fun-style curve adapter, `ReportRegistry` contract, `registry` client, `share` page, `publish` command. Stretch: Anvil-fork parity against Nad.fun's real contracts, AI red-team. Submission due **Oct 13, 2026**.
