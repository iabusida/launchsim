...
7. [x] **`packages/registry`** (done 2026-09-23). `toReportHash` (reuses `report`'s canonicalizer, one implementation, per docs/12), `buildRecordCall` (never signs), `readRecord`. 12 unit tests with a fake viem `custom` transport (`encodeFunctionResult` to fake `eth_call` responses -- no network), plus `abi.drift.test.ts` comparing `src/abi.ts` against the live `contracts/out/` artifact. A real Anvil integration test (`pnpm test:integration`, now wired at the root): spawns `anvil`, deploys the actual compiled bytecode, sends a real `record` transaction, reads the event log and the record back, and asserts an unrecorded hash reads back `null` -- genuine end-to-end proof, not mocked.
8. [x] **`packages/share`** (done 2026-09-23). Hono app: `GET /r/:id` (HTML report + chain record panel), `GET /r/:id/result.json`, `GET /badge/:id.png` (real PNG via `@resvg/resvg-js`, not a placeholder). `ChainRecordPanel`'s three states (recorded/not-recorded/mismatch) tested with a fake viem transport; the "mismatch" state is a defensive check (compares the on-chain record's own `scenarioHash` against the loaded report's) since a tampered report is actually content-addressed to a *different*, unrecorded hash, not a "mismatch" at the same hash -- documented in its TSDoc. Also added `parseRunResultJson` to `report` (the reverse of `toCanonicalJson`, needed so a stored `result.json` reads back with real `bigint`s, not stringified ones). **Verified with a full real-infrastructure smoke test, not just unit tests:** ran `launchsim run`, started a real `anvil`, deployed the actual compiled `ReportRegistry`, started `launchsim serve`, fetched all three routes (confirmed "Not recorded"), then recorded the report for real via `buildRecordCall` + a real transaction, and re-fetched `/r/:id` to confirm it now shows "Recorded on Monad" with the correct submitter address. **Known gap:** `cli run`'s output (`launchsim-report/result.json`, a single fixed path) and `ReportStore`'s expected layout (`<dir>/<runId>/result.json`, keyed by content hash) don't line up yet -- bridging them is `cli publish`'s job, not yet built (next).
