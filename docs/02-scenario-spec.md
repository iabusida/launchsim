...
- Amounts like `"2 SOL"` convert to lamports (`bigint`). Ranges like `"0.1-1 SOL"` become `{ min, max }` sampled with the seeded RNG.
...
- Note: on Solana, a token cannot burn from a DEX-owned vault without the Token-2022 permanent-delegate extension, which scanners flag as high risk. Math mode models the economics regardless; the report must mention this.

### `feeBuyback` — fee-funded buyback and burn (the fixed version)

Trading fees accrue in quote (SOL). Every `interval`, the mechanic withdraws accumulated fees, **buys** tokens from the pool with them, and burns the purchased tokens.
