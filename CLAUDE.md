...
├── indexer/                 # Envio HyperIndex project: indexes ReportRecorded, backs share's /reports
...
`testkit` (a devDependency-only fixture package) is the only piece that doesn't exist as real code today; every other package -- `core`, `adapters`, `report`, `cli`, `mcp`, `registry`, `share`, `contracts/` (a Foundry project, `ReportRegistry.sol` built, tested, and deployed to Monad testnet), and `indexer/` (an Envio HyperIndex project, **live on Envio Cloud**, synced 100% against Monad testnet via HyperSync) -- is built. Dependency direction is one-way: `cli`/`mcp`/`share` → `report`/`adapters`/`registry` → `core` (`mcp` also depends on `cli` directly, reusing its `runScenario`/`runCommand` rather than a second implementation). `contracts/` and `indexer/` are both independent, outside the pnpm workspace, same as each other; `registry` consumes its ABI from Foundry's build output (a copy in `src/abi.ts`, drift-tested against `contracts/out/`), and `indexer/abis/ReportRegistry.json` is a second, separate copy of that same ABI (see `indexer/README.md`). `core` depends on nothing internal. `testkit` is a devDependency only.
...
- `indexer/` (new, post-Phase-B): a standalone Envio HyperIndex project indexing `ReportRegistry.ReportRecorded` on Monad testnet via HyperSync (natively supported, no RPC-polling fallback needed), backing `share`'s new `GET /reports` route so every recorded report is browsable without already knowing its hash. Handler logic is tested with Envio's `createTestIndexer` (real handler, synthetic event, no network); `packages/share` stays at 100% coverage with `reportsIndex` as an optional dependency, so a deployment without the indexer running shows an honest "not configured" page rather than a crash or fake data. **Live on Envio Cloud** (free/development plan) at `https://indexer.dev.hyperindex.xyz/81a4159/v1/graphql`, verified end to end: synced 100% against Monad testnet, and the real `createEnvioReportsIndexClient` correctly reads both live-recorded reports back through it. A development-plan endpoint, not a pinned production URL -- Envio's free tier can retire it after sustained inactivity.
