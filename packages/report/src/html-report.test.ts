    ...
    checks: [
      {
        ...
        passed: false,
        summary: "pool quote fell to 18% of peak at hour 31",
        observed: "1800",
        threshold: "5000",
        atSlot: 279_000,
      },
...
describe("renderHtmlReport", () => {
  it("is a self-contained HTML document with no external scripts, fonts, or CDNs", () => {
    const html = renderHtmlReport(result(), { command: "launchsim run scenarios/x.ts", quoteUnit: { symbol: "MON", decimals: 18 } });
  ...
  it("shows a fail badge when passed is false and a pass badge when true", () => {
    expect(renderHtmlReport(result({ passed: false }), { command: "x", quoteUnit: { symbol: "MON", decimals: 18 } })).toContain("FAIL");
    expect(renderHtmlReport(result({ passed: true }), { command: "x", quoteUnit: { symbol: "MON", decimals: 18 } })).toContain("PASS");
  });

  it("lists every check with its summary", () => {
    const html = renderHtmlReport(result(), { command: "x", quoteUnit: { symbol: "MON", decimals: 18 } });
    expect(html).toContain("pool quote fell to 18% of peak at hour 31");
  });
  ...
  it("formats a positive pnl without a leading minus sign", () => {
        ...
        trades: [
          {
            ...
            group: "sniper",
            side: "buy",
            quote: 100_000_000_000_000_000n,
            base: 90n,
            ok: true,
            reason: null,
          },
          {
            ...
            group: "sniper",
            side: "sell",
            quote: 500_000_000_000_000_000n,
            base: 90n,
            ok: true,
            reason: null,
          },
    ...
    // pnl = 500_000_000_000_000_000 - 100_000_000_000_000_000 = 400_000_000_000_000_000 wei = 0.40 MON, positive
    expect(html).toContain("0.40 MON");
    expect(html).not.toContain("-0.40 MON");
  });
  ...
  it("includes the reproduce command and scenario hash", () => {
    const html = renderHtmlReport(result(), { command: "launchsim run scenarios/hourly-burn-lp.ts", quoteUnit: { symbol: "MON", decimals: 18 } });
  ...
  it("escapes untrusted scenario names to prevent XSS (docs/10)", () => {
    const html = renderHtmlReport(result({ scenario: { name: "<script>alert(1)</script>", hash: "h", seed: 1 } }), {
      command: "x",
      quoteUnit: { symbol: "MON", decimals: 18 },
    });
